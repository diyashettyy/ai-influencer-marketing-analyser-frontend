...
function ResultsContent() {
 ...
 const [influencers, setInfluencers] = React.useState<any[]>([])
  ...
  const availableCount = Math.min(requestedCount, influencers.length)
  ...
  useEffect(() => {
    ...
    const handlePopState = () => { router.replace('/campaign-setup') }
  ...
  React.useEffect(() => {
    // ── read from localStorage first — survives page refresh ────────────
    const cached = localStorage.getItem('latest_campaign_results')
    const resultId = localStorage.getItem('latest_result_id') || ''

    if (cached) {
      try {
        const parsed = JSON.parse(cached)
        const mapped = parsed.slice(0, count).map((inf: any, i: number) => {
          const name = inf.full_name || inf.username
          return {
            id         : i,
            campaignId : resultId,
            name       : name,
            handle     : inf.username ? '@' + inf.username : '@' + name,
            category   : inf.category || 'General',
            followers  : inf.followers || 0,
            aiScore    : inf.final_score ? Number(inf.final_score.toFixed(2)) : 0,
    ...
    // ── fallback only if localStorage is empty ───────────────────────────
    fetch('http://127.0.0.1:5000/results')
      .then(res => res.json())
      .then(data => {
        const latestCampaign = data.results?.[0]
        if (!latestCampaign || !latestCampaign.results) {
          setInfluencers([])
          setLoading(false)
          return
        }

        localStorage.setItem('latest_campaign_results', JSON.stringify(latestCampaign.results))
        localStorage.setItem('latest_result_id', latestCampaign._id)

        const mapped = latestCampaign.results.slice(0, count).map((inf: any, i: number) => {
          const name = inf.full_name || inf.username
          return {
            id         : i,
            ...
            name       : name,
            handle     : inf.username ? '@' + inf.username : '@' + name,
            category   : latestCampaign.category || 'General',
            followers  : inf.followers || 0,
            aiScore    : inf.final_score ? Number(inf.final_score.toFixed(2)) : 0,
            engagement : inf.engagement_score_norm ? Number(inf.engagement_score_norm.toFixed(2)) : 0,
          }
  ...
  if (influencers.length === 0) {
    return <div className="text-center mt-20">No influencers found</div>
  }
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
                    ...
                    <div className="bg-background rounded-xl p-4 border-2 border-border shadow-[3px_3px_0px_0px_var(--border)] text-center group-hover:shadow-[4px_4px_0px_0px_var(--border)] transition-shadow">
                      ...
                      <p className="font-serif text-2xl font-bold text-primary">{hero.aiScore}</p>
                    ...
                    <div className="bg-background rounded-xl p-4 border-2 border-border shadow-[3px_3px_0px_0px_var(--border)] text-center group-hover:shadow-[4px_4px_0px_0px_var(--border)] transition-shadow">
                      ...
                      <p className="font-serif text-2xl font-bold text-foreground">{hero.engagement}</p>
                  ...
                  {hero.description && (
                    <div className="bg-primary/5 rounded-xl p-4 border border-primary/20 mb-6 -rotate-[0.3deg]">
                      <div className="flex gap-2 items-start">
                        ...
                        <p className="text-sm text-foreground/80 italic leading-relaxed">{hero.description}</p>
                      </div>
                    </div>
                  )}
                </div>
                <div className="flex gap-3">
                  {/* fixed: uses campaignId + index as query param */}
                  <Link href={`/influencer-history/${hero.handle.replace('@', '')}`}
                  ...
                  <Link href= {`/influencer-history/${hero.handle.replace('@', '')}#collaborations`}
                    className="flex items-center justify-center px-4 bg-card text-foreground border-2 border-foreground rounded-xl hover:bg-muted transition-colors" title="View Collaborations">
                ...
                <div className={`relative ${colors.bg} rounded-2xl border-2 ${colors.border} p-7 shadow-[6px_6px_0px_0px_var(--border)] hover:shadow-[8px_8px_0px_0px_var(--border)] hover:-translate-y-1 transition-all duration-300 flex flex-col h-full`}>
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    ...
                    <div className="bg-card rounded-xl p-3 text-center border-2 border-border shadow-[2px_2px_0px_0px_var(--border)]">
                      ...
                      <p className="font-serif text-xl font-bold text-primary">{influencer.aiScore}</p>
                  ...
                  {influencer.description && (
                    <div className={`${colors.accent} border rounded-xl p-3 mb-5 flex-grow`}>
                      <div className="flex gap-2 items-start">
                        ...
                        <p className="text-xs text-foreground/75 italic leading-relaxed">{influencer.description}</p>
                      </div>
                    </div>
                  )}
                  <div className="flex gap-2 mt-auto">
                    {/* fixed: uses campaignId + index as query param */}
                    <Link href={`/influencer-history/${influencer.handle.replace('@', '')}`}
                    ...
                    <Link href={`/influencer-history/${influencer.handle.replace('@', '')}#collaborations`}
                      className="flex items-center justify-center p-2.5 bg-card text-foreground border-2 border-foreground rounded-xl hover:bg-muted transition-colors" title="View Collaborations">
                      <History className="w-5 h-5" />
                    </Link>
                  </div>
                </div>
        ...
        <div className="relative text-center bg-card rounded-3xl border-2 border-border p-12 shadow-[8px_8px_0px_0px_var(--border)] overflow-hidden">
          ...
          <Link href="/campaign-setup"
            onClick={() => localStorage.removeItem('latest_campaign_results')}
